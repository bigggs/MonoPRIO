...
from lib.models.monoprio_router import MonoPRIORouter


def _parse_class_weight_spec(spec, num_classes=3):
    # class order: [Pedestrian, Car, Cyclist]
    out = [1.0] * num_classes
    if spec is None:
        return out
    if isinstance(spec, (int, float)):
        return [float(spec)] * num_classes
    if isinstance(spec, (list, tuple)):
        if len(spec) != num_classes:
            raise ValueError(f"Expected {num_classes} class weights, got {len(spec)}")
    ...
    if isinstance(spec, dict):
        ...
        for k, v in spec.items():
            kk = str(k).lower() if not isinstance(k, int) else k
            if kk not in key_to_idx:
                raise ValueError(f"Unknown class key in class-weight spec: {k}")
            out[key_to_idx[kk]] = float(v)
        return out
    raise TypeError(f"Unsupported class-weight spec type: {type(spec)}")
...
class MonoPRIO(nn.Module):
  """ This is the MonoDGP module that performs monocualr 3D object detection """
    def __init__(self, backbone, depth_predictor, det2d_transformer, det3d_transformer,
                  ...
                  use_monoprio=False, prior_path="priors/train/priors_unified.npz",
                  monoprio_size_scale=1.0, monoprio_sigma_scale=0.0,
                  monoprio_class_size_scales=None):
        ...
        self.use_monoprio = bool(use_monoprio)
        self.prior_path = prior_path
        self.monoprio_size_scale = float(monoprio_size_scale)
        self.monoprio_sigma_scale = float(monoprio_sigma_scale)
        class_scales = _parse_class_weight_spec(monoprio_class_size_scales, num_classes=3)
        self.register_buffer(
            "monoprio_class_size_scales",
            torch.tensor(class_scales, dtype=torch.float32),
            persistent=False,
        )
        if self.use_monoprio:
            try:
                self.router = MonoPRIORouter(prior_path=self.prior_path, input_dim=hidden_dim)
            except FileNotFoundError as e:
                raise FileNotFoundError(f"prior file not found: {self.prior_path}") from e
        else:
            self.router = None
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        bank_mu_log = None
        bank_V_log = None
        bank_inv_std_log = None
        class_probs = None
        if self.use_monoprio:
            class_logits = inter_class[-1]  # [B, Q, C]
            p = torch.sigmoid(class_logits)
            class_probs = p / torch.clamp(p.sum(-1, keepdim=True), min=1e-6)
            class_probs = class_probs.detach()

            (prior_mu, prior_sigma, router_attn,
             bank_mu_log, bank_V_log, bank_inv_std_log) = self.router(
                hs_2d[-1], class_probs=class_probs
            )
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.use_monoprio and (prior_mu is not None):
                prior_mu_safe = torch.clamp(prior_mu, min=1e-3)

                scale = self.monoprio_size_scale
                if class_probs is not None:
                    class_scale = torch.matmul(
                        class_probs.to(self.monoprio_class_size_scales.dtype),
                        self.monoprio_class_size_scales
                    )
                    scale = scale * class_scale

                if (prior_sigma is not None) and (self.monoprio_sigma_scale > 0.0):
                    sigma = torch.clamp(prior_sigma, min=1e-3)
                    sigma_weight = 1.0 / (1.0 + sigma / self.monoprio_sigma_scale)
                    if torch.is_tensor(scale) and scale.ndim == sigma_weight.ndim - 1:
                        scale = scale.unsqueeze(-1)
                    scale = scale * sigma_weight

                if torch.is_tensor(scale):
                    if scale.ndim == prior_mu_safe.ndim - 1:
                        scale = scale.unsqueeze(-1)
                    size3d = torch.exp(size3d + scale * torch.log(prior_mu_safe))
                else:
                    size3d = torch.exp(size3d + float(scale) * torch.log(prior_mu_safe))

            size3d = torch.clamp(size3d, min=0.1)
        ...
        if self.use_monoprio:
            out['prior_mu'] = prior_mu
            out['prior_sigma'] = prior_sigma
            out['router_attn'] = router_attn
            out['bank_mu_log'] = bank_mu_log
            out['bank_V_log'] = bank_V_log
            out['bank_inv_std_log'] = bank_inv_std_log
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, inter_losses, group_num=11,
                 cap_decay_start=100, cap_decay_end=200, cap_decay_min=0.1,
                 cap_class_loss_weights=None):
        ...
        self.cap_decay_start = int(cap_decay_start)
        self.cap_decay_end = int(cap_decay_end)
        self.cap_decay_min = float(cap_decay_min)
        cap_cls_w = _parse_class_weight_spec(cap_class_loss_weights, num_classes=3)
        self.register_buffer(
            "cap_class_loss_weights",
            torch.tensor(cap_cls_w, dtype=torch.float32),
            persistent=False,
        )
    ...
    def loss_priors(self, outputs, targets, indices, num_boxes, epoch=0, **kwargs):
        """
        CAP loss on matched positives only.
        """
        losses = {}
        required = {'router_attn', 'bank_mu_log', 'bank_V_log', 'bank_inv_std_log', 'pred_3d_dim'}
        if not required.issubset(outputs.keys()):
            return losses

        idx = self._get_src_permutation_idx(indices)
        matched_attn = outputs['router_attn'][idx]      # [M, K]
        pred_dims = outputs['pred_3d_dim'][idx]         # [M, 3]
        log_pred = torch.log(torch.clamp(pred_dims, min=1e-4))

        bank_mu_log = outputs['bank_mu_log']            # [K, 3]
        bank_V_log = outputs['bank_V_log']              # [K, 3, 3]
        bank_inv_std_log = outputs['bank_inv_std_log']  # [K, 3]

        target_labels = torch.cat([t['labels'][i] for t, (_, i) in zip(targets, indices)], dim=0).long().view(-1)
        cls_w_cap = self.cap_class_loss_weights[target_labels].detach()

        diff = log_pred.unsqueeze(1) - bank_mu_log.unsqueeze(0)          # [M, K, 3]
        proj = torch.einsum('mki,kij->mkj', diff, bank_V_log)            # [M, K, 3]
        whitened = proj * bank_inv_std_log.unsqueeze(0)                  # [M, K, 3]
        md2 = torch.sum(whitened ** 2, dim=-1)                           # [M, K]

        # Short detach warmup to stabilize early routing updates.
        if epoch < 20:
            attn_cap = matched_attn.detach()
        elif epoch < 40:
            alpha = (epoch - 20) / 20.0
            attn_cap = matched_attn * alpha + matched_attn.detach() * (1.0 - alpha)
        else:
            attn_cap = matched_attn

        loss_cap_raw = torch.sum(attn_cap * md2, dim=-1)                 # [M]

        decay_factor = 1.0
        if epoch > self.cap_decay_start:
            duration = max(1, self.cap_decay_end - self.cap_decay_start)
            progress = min(1.0, (epoch - self.cap_decay_start) / duration)
            decay_factor = 1.0 + (self.cap_decay_min - 1.0) * progress

        losses['loss_cap'] = (loss_cap_raw * cls_w_cap).sum() / num_boxes * decay_factor
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        ...
        loss_map = {
            ...
            'priors': self.loss_priors,
        }

        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def forward(self, outputs, targets, mask_dict=None, epoch=0):
        ...
        for loss in self.losses:
            kwargs = {'epoch': epoch} if loss == 'priors' else {}
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss in ['depth_map', 'region', 'priors']:
                        continue
                    kwargs = {}
                    if loss == 'labels':
...
def build(cfg):
    ...
    model = MonoPRIO(
        ...
        use_monoprio=cfg.get('use_monoprio', False),
        prior_path=cfg.get('prior_path', 'priors/train/priors_unified.npz'),
        monoprio_size_scale=cfg.get('monoprio_size_scale', 1.0),
        monoprio_sigma_scale=cfg.get('monoprio_sigma_scale', 0.0),
        monoprio_class_size_scales=cfg.get('monoprio_class_size_scales', None))
    ...
    if cfg.get('use_monoprio', False):
        weight_dict['loss_cap'] = cfg.get('cap_loss_coef', 0.0)
    ...
    criterion = SetCriterion(
        ...
        group_num=cfg['group_num'],
        cap_decay_start=cfg.get('cap_decay_start', 100),
        cap_decay_end=cfg.get('cap_decay_end', 200),
        cap_decay_min=cfg.get('cap_decay_min', 0.1),
        cap_class_loss_weights=cfg.get('cap_class_loss_weights', None),
        )
